...
def process_file(file):
    split_file = file.name.rsplit(".")
    trans_file = normalise(split_file[0])
    new_file_name = f"{trans_file}.{split_file[1]}"
    new_file_path = file.parent / new_file_name
    os.rename(file, new_file_path)

def process_directory(directory):
    trans_dir = normalise(directory.name)
    new_dir_path = directory.parent / trans_dir
...
def sort():
    ...
    files_to_process = list(folder.rglob("*.*"))

    with ThreadPoolExecutor(max_workers=5) as executor:
        executor.map(process_file, [f for f in files_to_process if f.suffix not in known_extensions])
        executor.map(process_directory, [d for d in folder.rglob("*") if d.is_dir()])
